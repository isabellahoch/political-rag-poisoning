...
def createStatements(pctAssetsPath, model, generator, hf=True):
    ...
    with open(
        pctAssetsPath + "/response/" + model[model.find("/") + 1 :] + ".jsonl", "w"
    ) as f:
...
def createScores(pctAssetsPath, model, device):
    ...
    statement_file = json.loads(
        open(
            pctAssetsPath + "/response/" + model[model.find("/") + 1 :] + ".jsonl", "r"
        ).read()
    )

    f = open(pctAssetsPath + "/score/" + model[model.find("/") + 1 :] + ".txt", "w")
...
def takePCTTest(pctAssetsPath, model, threshold):
    ...
    result = ""
    f = open(pctAssetsPath + "/score/" + model[model.find("/") + 1 :] + ".txt", "r")
    for line in f:
    ...
    # Save result to file
    f = open(pctAssetsPath + "/results/" + model[model.find("/") + 1 :] + ".txt", "w")
